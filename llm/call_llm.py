...
def get_completion(
    ...
    if provider == "openai":
        ...
        return get_completion_spark(
            prompt, model, temperature, api_key, appid, api_secret, max_tokens
        )
    elif provider == "zhipuai":
        return get_completion_glm(prompt, model, temperature, api_key, max_tokens)
    elif provider == "gemini":
        return get_completion_gemini(prompt, model, temperature, api_key, max_tokens)
    else:
        return "不正确的模型"


def get_completion_gpt(
    prompt: str, model: str, temperature: float, api_key: str, max_tokens: int
):
    # 封装 OpenAI 原生接口
    if api_key == None:
...
def get_completion_wenxin(
    prompt: str, model: str, temperature: float, api_key: str, secret_key: str
):
    # 封装百度文心原生接口
...
def get_completion_spark(
    ...
    response = spark_main(
        appid, api_key, api_secret, Spark_url, domain, question, temperature, max_tokens
    )
    return response


def get_completion_glm(
    prompt: str, model: str, temperature: float, api_key: str, max_tokens: int
):
    # 获取GLM回答
    if api_key == None:
...
def parse_llm_api_key(model: str, env_file: dict() = None):
    ...
    if model == "openai":
        ...
        return env_file["GEMINI_API_KEY"]
    elif model == "wenxin":
